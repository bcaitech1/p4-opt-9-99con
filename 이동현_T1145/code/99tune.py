...
# def set_trial(trial, type, name, params):
#     temp = None
#     if type == 'categorical':
#         temp = getattr(trial, 'suggest_categorical',)
#     elif type == 'int':
#         temp = getattr(trial, 'suggest_int')
#     elif type == 'float':
#         temp = getattr(trial, 'suggest_float')
#     return temp
...
def search_hyperparam(
    ...
    img_size = trial.suggest_int("img_size", low=8, high=96, step=2)
    # img_size = trial.suggest_categorical("img_size", [8, 12, 16, 24, 32, 48, 64, 72,])
    n_select = trial.suggest_int("n_select", low=0, high=10, step=1)
    batch_size = trial.suggest_int("batch_size", low=16, high=128, step=16)
    val_ratio = trial.suggest_float('val_ratio', low=0, high=0.3)
    init_lr = trial.suggest_float('init_lr', low=1e-2, high=1e-1, step=2e-2)
    return {
        "EPOCHS": epochs,
        "IMG_SIZE": img_size,
        "n_select": n_select,
...
def objective(
    ...
    # [추가]
    model_config["depth_multiple"] = trial.suggest_float("depth_multiple", low=0.1, high=0.7)
    model_config["width_multiple"] = trial.suggest_float("width_multiple", low=0.1, high=0.7)
