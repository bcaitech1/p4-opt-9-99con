...
class TorchTrainer:
    ...
    def train(
        ...
        for epoch in range(n_epoch):
            ...
            with open(os.path.join(self.model_path[:-8], 'last_lr.yml'), 'w') as f:
                yaml.dump(self.scheduler.get_last_lr(), f, default_flow_style=False)
            ...
            save_model(
                model=self.model,
                path=self.model_path[:-3] + f"_{best_test_f1:.3f}.pt",
                data=data,
                device=self.device
            )
